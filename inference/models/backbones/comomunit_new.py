...
class Conv2dBlock(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size, stride, padding, norm='none', activation='relu', pad_type='zero'):
...
class AdaINGen(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, dim, style_dim, n_downsample, n_res, activ, pad_type, mlp_dim):
        ...
        vector_dim = 10
        self.dec = DecoderNoAdain(n_downsample, n_res, 16, output_dim, res_norm='adain', activ=activ, pad_type=pad_type)
        self.fc1 = nn.Linear(vector_dim, 128)
        self.fc2 = nn.Linear(128, 256)
        self.fc3 = nn.Linear(256, 16 * 120 * 160)
        self.fc = nn.Linear(vector_dim, 16 * 120 * 160)

    def forward(self, images):
        images_recon = self.decode(images)
        return images_recon
    ...
    def decode(self, content):
        ...
        content = content.view(content.size(0), 16, 120, 160)
...
class MsImageDis(nn.Module):
    ...
    def register_hooks(self):
        ...
        for cnn in self.cnns:
            cnn[-1].register_forward_hook(forward_hook)
            cnn[-1].register_backward_hook(backward_hook)
